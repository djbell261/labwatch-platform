...
public class KafkaConsumerConfig {
    ...
    @Value("${app.kafka.topic.alert-events-dlt:alert-events.notification-service.dlt}")
    private String alertEventsDltTopic;

    @Value("${app.kafka.topic.ai-investigation-events-dlt:ai-investigation-events.notification-service.dlt}")
    private String aiInvestigationEventsDltTopic;
    ...
    public ConsumerFactory<String, AiInvestigationEvent> aiInvestigationConsumerFactory() {
        return new DefaultKafkaConsumerFactory<>(baseConsumerProps(AiInvestigationEvent.class));
    }
    ...
    @Bean
    public CommonErrorHandler notificationKafkaErrorHandler(KafkaTemplate<String, Object> dltKafkaTemplate) {
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(
                dltKafkaTemplate,
                (record, exception) -> {
                    String dltTopic = "ai-investigation-events".equals(record.topic())
                            ? aiInvestigationEventsDltTopic
                            : alertEventsDltTopic;
    ...
    public NewTopic alertEventsDltTopic() {
        return TopicBuilder.name(alertEventsDltTopic)
                .partitions(3)
                .replicas(1)
                .build();
    }
    ...
    @Bean
    public NewTopic aiInvestigationEventsDltTopic() {
        return TopicBuilder.name(aiInvestigationEventsDltTopic)
                .partitions(3)
                .replicas(1)
                .build();
    }
    ...
    private final class LoggingRetryListener implements org.springframework.kafka.listener.RetryListener {
        ...
        public void failedDelivery(ConsumerRecord<?, ?> record, Exception ex, int deliveryAttempt) {
            log.warn(
                    "event=kafka_retry service=notification-service topic={} partition={} offset={} attempt={} error={}",
                    topic(record),
                    partition(record),
                    offset(record),
                    deliveryAttempt,
        ...
        public void recovered(ConsumerRecord<?, ?> record, Exception ex) {
            String dltTopic = "ai-investigation-events".equals(topic(record))
                    ? aiInvestigationEventsDltTopic
                    : alertEventsDltTopic;
            log.error(
                    "event=kafka_dlt service=notification-service topic={} partition={} offset={} dltTopic={} error={}",
                    topic(record),
                    partition(record),
                    offset(record),
                    dltTopic,
